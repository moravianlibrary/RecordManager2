...
public class KramAvailabilityReader implements ItemReader<List<KramAvailability>> {
	...
	private static final List<String> PAGE_URLS = Arrays.asList(
			...
			"%s/search?fl=dostupnost,dnnt,PID,level,dnnt-labels,parent_pid,details,document_type,issn&q=fedora.model:periodicalitem&rows=%d&start=%d&wt=xml",
	...
	public synchronized List<KramAvailability> read() {
		...
		while (reader.hasNext() && results.size() < BATCH_SIZE) {
			try {
				...
				if (!reader.hasNext() && availability == null && iterator.hasNext()) {
					url = iterator.next();
					start = 0;
					initializeReader();
				}
