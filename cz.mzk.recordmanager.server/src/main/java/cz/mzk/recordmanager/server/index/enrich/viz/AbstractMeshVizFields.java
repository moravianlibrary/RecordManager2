...
public abstract class AbstractMeshVizFields extends AbstractVizFields {
	...
	private static final int LRU_CACHE_SIZE_FIELD_450 = 20000;
	private static final int LRU_CACHE_SIZE_FIELD_451 = 400;
	private static final int LRU_CACHE_SIZE_FIELD_455 = 200;
	...
	protected List<String> getEnrichingValues(String key, String enrichingField) {
		if (config == null) {
			List<ImportConfiguration> configs = configDao
					.findByIdPrefix(Constants.PREFIX_MESH);
			...
			if (tr != null) {
				...
				for (DataField df : mr.getDataFields(enrichingField)) {
					if (df.getSubfield('a') != null
							...
							&& df.getSubfield('7').getData().startsWith("cze")) {
