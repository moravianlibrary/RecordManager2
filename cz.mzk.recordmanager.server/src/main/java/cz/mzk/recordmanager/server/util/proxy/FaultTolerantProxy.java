...
public class FaultTolerantProxy<T> implements InvocationHandler {
	...
	protected FaultTolerantProxy(T target, int maxRetries, int sleepBetweenFailures) {
		super();
		this.target = target;
	...
	public static <T> T create(T target, int maxRetries, int sleepBetweenFailures) {
		return (T) java.lang.reflect.Proxy.newProxyInstance(target.getClass().getClassLoader(),
				target.getClass().getInterfaces(), new FaultTolerantProxy<T>(target, maxRetries, sleepBetweenFailures));
	}
