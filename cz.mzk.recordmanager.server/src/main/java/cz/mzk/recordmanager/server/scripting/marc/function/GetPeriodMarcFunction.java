...
public class GetPeriodMarcFunction implements MarcRecordFunctions {
	 ...
	 */
	public Set<String> getPeriod(MarcFunctionContext ctx) {
		...
		if (period.length() == 4) {
			...
			return getPeriod(result, periodStart1, periodStart2, periodEnd1,
					periodEnd2);
			...
			if (gap == ' ' || gap == '-') {
				return getPeriod(result, periodStart1, periodStart2,
	 ...
	 *  from the 045a.
	 */
	private Set<String> getPeriod(Set<String> result, char periodStart1,
			char periodStart2, char periodEnd1, char periodEnd2) {
		if (periodStart1 == 'a' && periodStart2 == '0') {
			result.add(getPeriod('a', 0));
			periodStart1 = 'b';
		}
		if (periodStart1 >= 'a' && periodStart1 <= 'y' && periodEnd1 >= 'a'
				&& periodEnd1 <= 'y') {
			for (char periodVal = periodStart1; periodVal <= periodEnd1; periodVal++) {
				if (periodStart2 != '-' || periodEnd2 != '-') {
					char loopStart = (periodVal != periodStart1) ? '0'
							: Character.isDigit(periodStart2) ? periodStart2
									: '0';
					char loopEnd = (periodVal != periodEnd1) ? '9' : Character
							.isDigit(periodEnd2) ? periodEnd2 : '9';
					for (char periodVal2 = loopStart; periodVal2 <= loopEnd; periodVal2++) {
						result.add(""
								+ getPeriod(periodVal,
										Character.getNumericValue(periodVal2)));
					}
				} else {
					result.add("" + getPeriod(periodVal, 0));
				}
			}
		}
		return result;
