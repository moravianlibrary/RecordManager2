DROP TABLE IF EXISTS tmp_simmilar_articles_xg;
DROP TABLE IF EXISTS tmp_articles_xg_keys;
...
WHERE t.order_in_record = 1
  ...
  AND hr.source_info_x IS NOT NULL;
...
FROM harvested_record hr
INNER JOIN title t ON hr.id = t.harvested_record_id
INNER JOIN tmp_articles_xg_keys k
   ON k.source_info_x = hr.source_info_x
  AND k.source_info_g IS NOT DISTINCT FROM hr.source_info_g
...
WHERE t.order_in_record = 1
GROUP BY t.title, hr.author_string, hr.publication_year, hr.source_info_x, hr.source_info_g
...
DROP TABLE tmp_articles_xg_keys;

CREATE INDEX tmp_articles_xg_idx ON tmp_simmilar_articles_xg(row_id);