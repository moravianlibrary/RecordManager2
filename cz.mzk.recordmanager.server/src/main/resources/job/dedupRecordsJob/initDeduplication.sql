...
CREATE UNLOGGED TABLE tmp_periodicals_ids AS
...
CREATE UNLOGGED TABLE tmp_audio_ids AS
SELECT hrfl.harvested_record_id AS id FROM harvested_record_format_link hrfl 
  INNER JOIN harvested_record_format hrf ON hrf.id = hrfl.harvested_record_format_id
...
CREATE UNLOGGED TABLE tmp_video_ids AS
SELECT hrfl.harvested_record_id AS id FROM harvested_record_format_link hrfl
  INNER JOIN harvested_record_format hrf ON hrf.id = hrfl.harvested_record_format_id
...
CREATE UNLOGGED TABLE tmp_sfx_conf_ids AS
...
CREATE UNLOGGED TABLE tmp_sfx_ids AS
