...
WHERE hr.disadvantaged IS TRUE AND hr.author_string IS NOT NULL AND hr.publisher IS NOT NULL AND hr.publication_year IS NOT NULL
GROUP BY t.short_title,hr.author_string,hr.publisher,hr.publication_year
...
CREATE INDEX tmp_disadvantaged_publisher_idx ON tmp_disadvantaged_publisher(row_id);