DROP TABLE IF EXISTS tmp_bls_author_title_lang;

CREATE TABLE tmp_bls_author_title_lang AS
...
WHERE hr.bl_author IS NOT NULL AND hrfl.harvested_record_format_id in (1,16,13,12,14,15,17)
GROUP BY hr.bl_author, titles.title, l.lang
HAVING COUNT(DISTINCT biblio_linker_id)>1
  AND bool_or(next_biblio_linker_flag) IS TRUE;

CREATE INDEX tmp_bls_author_title_lang_idx ON tmp_bls_author_title_lang(row_id);