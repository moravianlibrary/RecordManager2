DROP TABLE IF EXISTS tmp_bls_auth_common_title;

CREATE TABLE tmp_bls_auth_common_title AS
...
WHERE hr.bl_author_auth_key IS NOT NULL AND hrfl.harvested_record_format_id!=2
GROUP BY hr.bl_author_auth_key, ct.title, l.lang, hrfl.harvested_record_format_id
HAVING COUNT(DISTINCT biblio_linker_id)>1
  AND bool_or(next_biblio_linker_flag) IS TRUE;

CREATE INDEX tmp_bls_auth_common_title_idx ON tmp_bls_auth_common_title(row_id);