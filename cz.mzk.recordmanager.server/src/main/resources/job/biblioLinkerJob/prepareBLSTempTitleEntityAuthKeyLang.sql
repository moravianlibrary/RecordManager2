DROP TABLE IF EXISTS tmp_bls_title_entity_auth_key_lang;

CREATE TABLE tmp_bls_title_entity_auth_key_lang AS
...
GROUP BY e.entity_auth_key, titles.title, l.lang
HAVING COUNT(DISTINCT biblio_linker_id)>1
  AND bool_or(next_biblio_linker_flag) IS TRUE;

CREATE INDEX tmp_bls_title_entity_auth_key_lang_idx ON tmp_bls_title_entity_auth_key_lang(row_id);