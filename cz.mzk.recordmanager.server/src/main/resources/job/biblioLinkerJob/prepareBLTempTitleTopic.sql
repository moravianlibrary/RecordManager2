DROP TABLE IF EXISTS tmp_bl_title_topic;

CREATE TABLE tmp_bl_title_topic AS
...
WHERE hr.bl_author IS NULL AND hr.bl_author_auth_key IS NULL AND dedup_record_id IS NOT NULL AND hrfl.harvested_record_format_id IN (1,4,5)
GROUP BY titles.title, hr.bl_topic_key, l.lang, hrfl.harvested_record_format_id
HAVING COUNT(*)>1
...
CREATE INDEX tmp_bl_title_topic_idx ON tmp_bl_title_topic(row_id);