DROP TABLE IF EXISTS tmp_bls_auth_key_title;

CREATE TABLE tmp_bls_auth_key_title AS
...
WHERE hr.bl_author_auth_key IS NOT NULL AND hrfl.harvested_record_format_id in (5,16,13,12,14,15,17)
GROUP BY hr.bl_author_auth_key, titles.title
HAVING COUNT(DISTINCT biblio_linker_id)>1
  AND bool_or(next_biblio_linker_flag) IS TRUE;

CREATE INDEX tmp_bls_auth_key_title_idx ON tmp_bls_auth_key_title(row_id);