DROP TABLE IF EXISTS tmp_bls_author_title_audio_book;

CREATE TABLE tmp_bls_author_title_audio_book AS
SELECT nextval('tmp_bl_id_seq') AS                                   row_id,
       ...
       hr.bl_author,
       titles.title
FROM (SELECT harvested_record_id,
             title
      FROM title
      UNION
      SELECT harvested_record_id,
             title
      FROM bl_title) titles
...
WHERE hr.bl_author IS NOT NULL
  AND hrfl.harvested_record_format_id in (1, 12, 13, 14, 15, 16, 17)
...
CREATE INDEX tmp_bls_author_title_audio_book_idx ON tmp_bls_author_title_audio_book (row_id);