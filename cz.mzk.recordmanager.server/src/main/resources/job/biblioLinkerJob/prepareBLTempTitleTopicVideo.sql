DROP TABLE IF EXISTS tmp_bl_title_topic_video;

CREATE TABLE tmp_bl_title_topic_video AS
...
WHERE hr.bl_author IS NULL AND hr.bl_author_auth_key IS NULL AND hr.bl_topic_key IS NOT NULL AND dedup_record_id IS NOT NULL
GROUP BY titles.title, hr.bl_topic_key
HAVING COUNT(*)>1
...
CREATE INDEX tmp_bl_title_topic_video_idx ON tmp_bl_title_topic_video(row_id);