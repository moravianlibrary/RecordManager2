DROP TABLE IF EXISTS tmp_bls_author_format_lang;

CREATE TABLE tmp_bls_author_format_lang AS
...
GROUP BY hr.bl_author, l.lang, hrfl.harvested_record_format_id
HAVING COUNT(DISTINCT biblio_linker_id)>1
  AND bool_or(next_biblio_linker_flag) IS TRUE;

CREATE INDEX tmp_bls_author_format_lang_idx ON tmp_bls_author_format_lang(row_id);